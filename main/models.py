...
class Category(models.Model):
    ...
    popularity = models.PositiveIntegerField(
        default=0, verbose_name="Популярность", blank=True
    )  # популярность товара
...
class Product(models.Model):
    ...
    name = models.CharField(
        max_length=100, db_index=True, verbose_name="Продукт"
    )  # название товара
    slug = models.SlugField(
        max_length=100, unique=True, verbose_name="url"
    )  # человекочитаемый URL
    image = models.ImageField(
        upload_to="images/products/", blank=True, verbose_name="Изображение"
    )  # изображение товара
...
class Service(models.Model):
    product = models.ForeignKey(
        Product, related_name="services", on_delete=models.CASCADE, verbose_name="Товар"
    )  # связь с товаром
    name = models.CharField(
        max_length=100, db_index=True, verbose_name="Услуга"
    )  # название услуги
    slug = models.SlugField(
        max_length=100, unique=True, verbose_name="url"
    )  # человекочитаемый URL
...
class Order(models.Model):
    ...
    image = models.ImageField(
        upload_to="images/orders/", blank=True, verbose_name="Фото"
    )
...
class JobApplication(models.Model):
    ...
    specialization = models.CharField(
        max_length=100, verbose_name="Специализация", db_index=True
    )
